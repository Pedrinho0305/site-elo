...
- **Uber:** `uber.js` implementa a Guest Rides API (token `client_credentials`, estimativa, pedido, consulta, cancelamento). Sem `UBER_CLIENT_ID/SECRET` roda **simulado** (corrida fictícia que avança sozinha). Fluxo da pochete: botão amarelo → corrida `pendente` → cuidador aprova no painel → estimativa + pedido → status sincronizado a cada 10 s.
- **Chamar Uber pelo painel** (`POST /api/corridas`, botão `#acaoUber`): o cuidador pede o carro sem esperar a pochete. A partida é a última posição da pochete ou a geolocalização do navegador (desktop e celular); o destino é o endereço de casa cadastrado. Com credenciais da Uber, pede pela Guest Rides API e o painel acompanha o status (`modo: "api"`). **Sem credenciais, não simula:** devolve o **link universal** `m.uber.com/ul` (`uber.linkUniversal`), que abre o app no celular e o site do Uber no computador com partida e destino preenchidos (`modo: "link"`) — é o único caminho que chama carro de verdade sem a Uber aprovar uma conta empresarial. Como o pedido termina dentro do Uber, esse modo não registra corrida: o servidor não teria como saber o que aconteceu depois.
